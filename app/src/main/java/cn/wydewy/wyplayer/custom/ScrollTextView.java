 ...
 * 
 * @author CWD
 * @version 2013.07.10 v1.0 实现动态刷新效果
 */
public class ScrollTextView extends TextView {
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawText(arrays.get(i), x, y1, paint);// 第一句
		canvas.drawText(arrays.get((i == size - 1) ? 0 : i + 1), x, y2, paint);// 第二句
	}
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		this.height1 = (float) h / 2 + (float) getPaddingTop()
				+ (float) getLineHeight() / 4;// 我也不知道为什么这样计算正好是中点
		this.height2 = (float) height1 * 2.5f;// 稍微多0.5的距离，这样不会留下尾巴
		reset();
	}
